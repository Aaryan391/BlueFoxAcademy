                            ...
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <nav aria-label="breadcrumb">
                                    <ol class="breadcrumb mb-0">
                                        <li class="breadcrumb-item"><a href="/teacherdashboard">Dashboard</a></li>
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    async function fetchAdmissions(page = 1) {
        try {
            const response = await axios.get(`/api/teacher/admissions?page=${page}`);
    ...
    async function fetchEnrollments(page = 1) {
        try {
            const response = await axios.get(`/api/teacher/enrollments?page=${page}`);
            ...
            document.getElementById('enrollmentsTableBody').innerHTML = enrollments.map(enrollment => `
                <tr>
                    ...
                    <td>
                        <select class="form-select form-select-sm enrollment-status w-auto" 
                            data-enrollment-id="${enrollment.id}"
                            data-original-value="${enrollment.status}"
                            ${enrollment.status === 'completed' ? 'disabled' : ''}>
    ...
    window.viewAdmissionDetails = async function(admissionId) {
        try {
            const response = await axios.get(`/api/teacher/admissions/${admissionId}`);
    ...
    document.addEventListener('change', async function(e) {
        if (e.target.classList.contains('enrollment-status')) {
        ...
        try {
            const response = await axios.post(`/api/teacher/enrollments/${enrollmentId}/status`, {
                status: newStatus
            });

            // Only update UI if the server request was successful
            if (response.data.success) {
                showAlert('enrollments-alert', 'Status updated successfully', 'success');
                if (newStatus === 'completed') {
                    e.target.disabled = true;
                }
                await fetchEnrollments(); // Refresh the table
            }
        } catch (error) {
            ...
            showAlert('enrollments-alert', errorMessage, 'danger');
        }
